...
public abstract class BlueToothBase {
	...
	protected UUID _uuid_secure =
        UUID.fromString("1658f56d-2dd4-4faf-bd34-2f8e5716fe80");
	protected UUID _uuid_insecure =
        UUID.fromString("40dd0626-305a-4c9b-bc76-378c14b173e3");
    ...
    protected void onMessageDeviceName(Message msg) {
		mConnectedDeviceName = msg.getData().getString(BlueToothMessageHandler.DEVICE_NAME);
		Toast.makeText(_caller.getApplicationContext(),
				"Connected to " + mConnectedDeviceName, Toast.LENGTH_SHORT)
				.show();
		Log.d(TAG, "MESSAGE_DEVICE_NAME: " + "Connected to " + mConnectedDeviceName);
    }
    ...
    protected void onMessageToast(Message msg) {
    	if(_caller != null) {
    		Toast.makeText(_caller.getApplicationContext(),
					msg.getData().getString(BlueToothMessageHandler.TOAST), Toast.LENGTH_SHORT).show();
    	}
	...
	protected synchronized void setState(int state) {
        ...
        mHandler.obtainMessage(BlueToothMessageHandler.MESSAGE_STATE_CHANGE, state, -1).sendToTarget();
    ...
    protected void connectionLost() {
        ...
        Message msg = mHandler.obtainMessage(BlueToothMessageHandler.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(BlueToothMessageHandler.TOAST, "Device connection was lost");
        msg.setData(bundle);
    ...
    protected void connectionFailed() {
        ...
        Message msg = mHandler.obtainMessage(BlueToothMessageHandler.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(BlueToothMessageHandler.TOAST, "Unable to connect device");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    }
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    mHandler.obtainMessage(BlueToothMessageHandler.MESSAGE_READ, bytes, -1, buffer)
                            .sendToTarget();
                } catch (IOException e) {
...
//                    connectionLost();
        ...
        public void write(byte[] buffer) {
            try {
                ...
                mHandler.obtainMessage(BlueToothMessageHandler.MESSAGE_WRITE, -1, -1, buffer)
                        .sendToTarget();
            } catch (IOException e) {
